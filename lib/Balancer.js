...
function Balancer(options) {
	...
	this.proxy.on('error', function(err, req, res) {
		log.error('proxy', 'http://' + req.headers.host + req.url + ' failed (' + err.message + ')');
...
Balancer.prototype.handleNodeRegister = function(req, res) {
	function fail(err) {
		log.error(req.connection.remoteAddress, 'Failed to register (' + err.message + ')');
	...
	this._getJSONBody(req, function(err, body) {
		...
		self.nodes.register(req.connection.remoteAddress, body, function(err) {
			if (err) return fail(err);
...
Balancer.prototype.handleNodeUnregister = function(req, res) {
	function fail(err) {
		log.error(req.connection.remoteAddress, 'Failed to unregister (' + err.message + ')');
		res.writeHead(500, {'Content-Type': 'text/plain'});
		res.write(err.message);
		return res.end();
	}
	...
	self.nodes.unregister(req.connection.remoteAddress, {id: id}, function(err) {
		if (err) return fail(err);
...
Balancer.prototype.close = function(callback) {
	function close(http) {
		return function(callback) {
			if (http) return http.close(callback);
			callback();
		};
	}
	async.parallel([
		close(this.http_private),
		close(this.http_public)
	], callback);
};
